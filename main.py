...
if __name__ == '__main__':
    ...
    try:
        ...
        if not auth or not auth.cookie or 's_v_web_id' not in auth.cookie:
            print("❌ 认证配置错误！")
            print("\n📋 请按照以下步骤配置认证信息：")
            print("1. 在项目根目录创建 .env 文件")
            print("2. 在 .env 文件中添加以下内容：")
            print("   DY_COOKIES=你的抖音Cookie")
            print("   DY_LIVE_COOKIES=你的抖音直播Cookie")
            print("\n🔍 获取Cookie的方法：")
            print("1. 在浏览器中登录抖音 (www.douyin.com)")
            print("2. 按F12打开开发者工具")
            print("3. 点击 Network 标签")
            print("4. 刷新页面或进行任何操作")
            print("5. 找到任意一个请求，复制其Cookie值")
            print("6. 将Cookie值粘贴到 .env 文件的 DY_COOKIES 中")
            print("\n⚠️  注意：必须登录后的Cookie才有效！")
            exit(1)
